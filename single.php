...
require_once 'db_conf.php';
	    ...
	    $res=$DB_con->exec("INSERT INTO commande(pro_id,user_id,qtee) values('$id','$user','$qte_com')");
	      }
		 }
       ...
       ?> 
                  ...
                  <form method="post">
					...
					<?php  
                      ...
                      if($prixsolde==0){
                        ...
                        $pourcentage='<span class="pcg">-'.$row['pourcentage'].'%</span>';
						...
						<script>
							...
							<?php 
							 $datefin=$row['datefinsolde'];
							?> 
							var countDownDate = new Date("<?php echo $datefin; ?>").getTime();
							...
							<?php echo $prix.$pourcentage.'<span>'.$prixsolde.'</span><span id="demo"></span>';?>
						...
						<input type="number"
						 <?php if(isset($_POST['quantite_com'])){
						 	echo 'value="'.$_POST['quantite_com'].'"';}else {echo 'value="1"';}
						?> name="quantite_com" min="1">
						<button type="sumbit" name="ajout-pro" class="btn-ajpanier">
						 <?php 
						    if(isset($_POST['ajout-pro'])){
						    	echo '<i class="fa fa-check" class="font-fa" ></i>';
						     }else{
						     	echo '<i class="fa fa-opencart" class="font-fa" ></i>';
						     }
						 ?>
						 Ajouter au panier
						</button> 
						 <?php
                           if(isset($_POST['addlist'])){
                           	$numpro=$_POST['addlist'];	
                           	$rep= $DB_con->query("SELECT user_id FROM users where user_email='$umail'");
                           	$donnes = $rep->fetch();
                           	$numuser=$donnes['user_id'];
                           	
                           	$date_cree=date("Y-m-d")." ".date("H:i:s");
                           	$res=$DB_con->exec("INSERT INTO mesliste(numpro,numuser,date_cree) 
    						  values('$numpro','$numuser','$date_cree')");
                           	
                           }
                           ?>
							
						<button type="submit" name="addlist" class="btn-aliste" <?php echo 'value="'.$row['id'].'"'; ?>>
							<?php 
						    if(isset($_POST['addlist'])){
						     ...
						     ?>  Ajouter à ma liste d'envies</button>	
						</div>
					</div>
				</form>	
					  ...
					  <div class="tab-content" style="background:transparent">
					    <div id="home" class="tab-pane fade in active">
					    	<table class="table" style="background:#fff;border-radius:4px;border:1px solid #ddd" >
					    		<tbody>
					    		<?php
					    		$reponse = $DB_con->query("SELECT *  FROM fichet where referance='$ref'");
		  ...
		  <ul class="nav nav-tabs">
		  	         <?php 	 
						...
						$donnees = $reponse->fetch();
		  	             ...
		  	             $reponse = $DB_con->query("SELECT count(id) as nbr FROM produit where categorie='$cat'");
						 $donnees = $reponse->fetch();
        ...
        <div class="carousel-inner">
            <div class="item active">
                    <ul class="thumbnails">
                    	<?php  
		  	             $reponse = $DB_con->query("SELECT * FROM produit where categorie='$cat' limit 4");
