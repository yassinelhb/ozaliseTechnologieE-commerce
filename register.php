...
$reponse = $DB_con->query("SELECT user_email,user_pass,user_name,personnalite FROM users where user_email='$umail' LIMIT 1");
$donnees=$reponse->fetch();
  $nb= $reponse->rowCount();
  if ($nb>0) {
   if($donnees['user_pass']==$upass){
   	  if($donnees['personnalite']=="admin"){
   	  	header('Location: admin.php'); 
   	  }else{
        header('Location: index.php');  
        $_SESSION['user_session'] = $donnees['user_email'];
       }
	...
	<div class="about" > 
		<div class="container">
			...
			<div class="col-md-8 about-center">
				...
				<div class="col-md-6 form-inscrie">
					<form method="post">
					<h3 class="title-form">Créez votre compte</h3>
					<p>Saisissez votre adresse e-mail pour créer votre compte.</p>
					<div id="error">
                                 <?php

                                if((isset($_POST['btnsignup']))&&(isset($error))) {
                                 ?>
                                 <div class="alert alert-danger">
					...
					<label>Adresse e-mail</label><input type="email" class="form-input" name="uname_email"><br/>
					
					<button type="sumbit" name="btnsignup">Créez votre compte</button>
					</form>
				...
				<div class="col-md-6 form-login">
					...
					<label>Adresse e-mail</label><input type="email" class="form-input" name="uname_email" 
					<?php if(isset($_POST['uname_email'])){ echo 'value="'.$_POST['uname_email'].'"';} ?>><br/>
					<label>Mot de passe</label><input type="password" class="form-input" name="uname_pass"><br/>
					<a href="">Mot de passe oublie ?</a><br/>
					<button type="sumbit" name="btn-login"> Connexion</button>
					</form>
