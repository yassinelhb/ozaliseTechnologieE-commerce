...
if (isset($_SESSION['user_session'])) {
 ...
 if($donnes['adresse']=="" || $donnes['ville']="" || $donnes['etat']=="" || $donnes['pays']=="" || $donnes['cod_postal']==0 || $donnes['num_tel']==0){
  ...
  <div class="about" > 
    <div class="container">
      ...
      <?php
        if(isset($_POST['pqte'])){
            $numcom= $_POST['pqte'];
           $DB_con->exec("UPDATE commande SET qtee=qtee + 1 where user_id='$numuser' and numcom=$numcom");
          }
          if(isset($_POST['mqte'])){
             $numcom= $_POST['mqte'];
           $DB_con->exec("UPDATE commande SET qtee=qtee - 1 where user_id='$numuser' and numcom=$numcom");
      ...
      <?php
        if(isset($_POST['delete_list'])){
          $numcom= $_POST['delete_list'];
              $DB_con->exec("DELETE FROM commande WHERE numcom=$numcom");
          }
        ?>
      <div class="col-md-8 about-center">
        ...
        <?php
          ...
          $reponse = $DB_con->query("SELECT user_name,user_prenom,societe,adresse,ville,etat,cod_postal,pays,num_tel
          FROM users where user_id='$numuser'");

           echo '<a href="index.php" class="btn-panier">Continuer mes achats</a>';
            echo '<h1 class="page-heading"><strong>1</strong> Adresses </h1>
                  <div class="col-md-12 form-insc">
                   ...
                   echo '<p>'.$donnees['user_name'].' '.$donnees['user_prenom'].'</p>
                         ...
                         <p>'.$donnees['pays'].' </p>
                         <p>'.$donnees['num_tel'].' </p>
                         <a href="adresse.php" class="btn-met">Mettre à jour</a>
